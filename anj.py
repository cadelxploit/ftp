...
def set_terminal_title():
    if os.name == 'nt':  # Jika Windows
        os.system('title FTP Checker by seclerx')
    else:  # Jika Linux/MacOS
        print("\033]0;FTP Checker by seclerx\007")
...
"""
    ...
    print(colored("--> Use format : host:user:password\n", "yellow"))

# Fungsi untuk mencoba login ke FTP dan menyimpan hasil sukses
def check_ftp_login(ftp_host, ftp_user, ftp_pass):
    ...
    try:
        with open(file, 'r', encoding='utf-8') as f:
            ...
            for line in f:
                ...
                parts = line.split(':')
                if len(parts) == 3:
                    ftp_host, ftp_user, ftp_pass = parts
                    check_ftp_login(ftp_host, ftp_user, ftp_pass)
                else:
...
if __name__ == "__main__":
    ...
    while True:
        file = input(colored("FTP list file: ", "cyan")).strip()
        if file.lower() == 'exit':
